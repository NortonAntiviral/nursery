...
class TypeController {
    async index({view, auth, request, response}){
        let pageRows = parseInt(request.all().rows);
        pageRows = request.all().hasOwnProperty('rows') ? pageRows : 10;
        let start = (parseInt(request.all().page) - 1) * pageRows;
        start  = request.all().hasOwnProperty('page') ? start : 1;       
        try{
            ...
            let totalPages = Math.round(tableRows / pageRows);
            let arrayOfPages = [];
            let i = 0;
